...
- **Language:** ALL code, comments, documentation, commit messages, and any text added to the codebase MUST be in **English**. This applies regardless of the conversation language or the model being used. The ONLY exception is when the user gives an explicit, specific directive to produce content in a different language (e.g., "write this document in Turkish"). Without such a directive, English is non-negotiable.
