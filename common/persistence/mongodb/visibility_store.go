...
func (s *visibilityStore) RecordWorkflowExecutionClosed(ctx context.Context, request *store.InternalRecordWorkflowExecutionClosedRequest) error {
	...
	if shouldUnsetTransient {
		updateOps["$unset"] = bson.M{searchattribute.TemporalPauseInfo: ""}
	}
...
func (s *visibilityStore) stripTransientSearchAttributes(doc *visibilityExecutionDocument) (bool, error) {
	...
	if len(doc.SearchAttributesMap) > 0 {
		if _, ok := doc.SearchAttributesMap[searchattribute.TemporalPauseInfo]; ok {
			delete(doc.SearchAttributesMap, searchattribute.TemporalPauseInfo)
			if len(doc.SearchAttributesMap) == 0 {
	...
	if _, ok := sa.IndexedFields[searchattribute.TemporalPauseInfo]; !ok {
		return removed, nil
	}

	delete(sa.IndexedFields, searchattribute.TemporalPauseInfo)
