...
func (s *executionStore) SetWorkflowExecution(
	...
	existing, err := s.loadExecutionDocument(ctx, snapshot.NamespaceID, snapshot.WorkflowID, runID)
	if err != nil {
		return err
	}
	if existing == nil {
		return &persistence.ConditionFailedError{Msg: "workflow execution not found"}
	}
	...
	result, err := s.executionsCol.UpdateOne(ctx, filter, update)
	if err != nil {
		return serviceerror.NewUnavailablef("failed to set workflow execution: %v", err)
	}
	if result.MatchedCount == 0 {
		return &persistence.ConditionFailedError{Msg: "workflow execution set conflict"}
	}

	return s.insertWorkflowTasks(ctx, request.ShardID, snapshot.Tasks)
}
